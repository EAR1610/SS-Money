eje = function(arrays,origen,redisClient) {
	...
	recibo tokens, dpi ,idasesor ,monto a prestar ,cicloendiad, fecha_prestamoNoSEUSA, descontarCUOTADEuNA, porcentajeDEPRESTAMO ,quedo_cuotauNICA, IDEMPRESA, dIARIOSEMANALMENSUAL, UltimaCuota,  C1, C2, C3, C4, contratosMultiplesAsesores, codigo_contrato]
		*/
		console.log(arrays)
		if ( arrays.length == 18 ){
			...
			jwt.verify(arrays[0], 'clWve-G*-9)1', function(err, decoded) {
				if (err) {
					...
					if( arrays[0]!==null && arrays[1] !==null && arrays[2] !==null && arrays[3] !==null && arrays[4] !==null && arrays[5] !==null && arrays[6] !==null && arrays[7] !==null && arrays[8] !==null && arrays[9] !==null && arrays[10] !==null ){
						...
						/**
						 * ? Función para generar un UUID (Universal Unique Identifier)
						 * @returns {string} - Un UUID en formato estándar.
						 */
						...
						*/
						...
						redisClient.get("configuracion_" + arrays[9], function (err, reply) {
							if( reply!==null && reply !== undefined ){
								...
								// ? Verificar si el codigo de contrato ya existe
								var patron = "registry_*_contrato_*_*_*";

								redisClient.keys(patron, function(errx, claves){
									if( errx ) return reject([false, "9"]);
									if( !Array.isArray(claves) ) claves = [];

									if( claves.length > 0 ){
										function iterar(ind, arrs){
											if( ind >= arrs.length ) { // * Si se aprueba la verificación del código del contrato, se procede a guardar el contrato. 
												crearContrato();
											} else { // * Se comprueba si el código ya existe en los contratos vigentes.
												redisClient.get(arrs[ind], function(errc, datac){													
													if( datac !== null && datac !== undefined ){
														try{
															var registro = JSON.parse(datac);
															if( registro[14] == arrays[17] && arrays[17] !== "" ){
																	extric = '9'; 
																	iterar(arrs.length, arrs);
															}
														} catch(e){}
													}
													iterar(ind+1, arrs);
									...
									function crearContrato(){
										redisClient.keys("registry_"+arrays[1]+"_contrato_"+arrays[0]+"_"+arrays[2]+"_*",function(ersr,replsy) {
													if( !Array.isArray(replsy) ) replsy = [];
													...
													redisClient.keys("registry_"+arrays[1]+"_contrato_*",function(erxsr,replxsy) {
														...
														*/
														...
														if( extric == "3" && miEmpresa > 0 ) {
															...
															resolve( [false, "9", 'El código de contrato ya está registrado'] );
														} else {
															...
															*/				
															...
															if( fes.length > 0 ){
																...
																*/	
																...
																// * Agregar el código de contrato al array
																arrays.push(arrays[17]);
																...
																redisClient.keys("registry_" + arrays[1] + "_contrato_*", function(err, keys) {
																	...
																	console.log("oriegn")
																	console.log(oriegn)
																	const arraysDB = arrays.slice(0, 11).concat(arrays.slice(18)); // ? Se ajusta los elementos del array del contrato. En caso de agregar datos al contrato, se suma el valor de .slice
